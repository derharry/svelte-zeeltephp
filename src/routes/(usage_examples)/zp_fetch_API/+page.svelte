<script lang="ts">
     ...
     import { zp_fetch, HTML_Marquee } from "zeeltephp"
...
<h1>zp_fetch context for API-requests</h1>
<p>Here you find showcases for API requests like SvelteKit.</p>

<div class="grid-container">

     <button on:click={() => {   promise = zp_fetch('/zpdev', { method: 'GET' })   }}>GET</button>
     <span>
          Execute GET
     </span>
     ...
     <button on:click={() => {   promise = zp_fetch('/zpdev', { method: 'HEAD', fetch: fetch }) }}>HEAD</button>
     <span>
          Execute HEAD
     </span>
